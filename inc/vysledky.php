...
<?php               } else {     ?>
                    <p>Celkové pořadí závodníků za všechny etapy tohoto ročníku.</p>
<?php               }        ?>
                  </div>
                </div>
              </div>
...
<?php
            ...
            $sql1 = "SELECT jmeno,SUM(body) as bodyCelkem,klub,stravaID,pohlavi FROM ucastnici WHERE (pohlavi='M' AND etapa IN (select id from etapy where akce=".$id_akce.")) GROUP BY jmeno,stravaID,klub ORDER BY bodyCelkem DESC";
...
<?php
                      $sql1 = "SELECT jmeno,SUM(body) as bodyCelkem,klub,stravaID,pohlavi FROM ucastnici WHERE (pohlavi='Z' AND etapa IN (select id from etapy where akce=".$id_akce.")) GROUP BY jmeno,stravaID,klub ORDER BY bodyCelkem DESC";
                      $result1 = mysqli_query($conn, $sql1);
                      $poradi = 1;
                      if (mysqli_num_rows($result1) > 0) { 
                            while($prvek = mysqli_fetch_assoc($result1)) {
                              if ($mamBody == true AND $pohlavi == "Z") {
                                if ($prvek["bodyCelkem"] < $mojeBodyCelkem) {
                                    echo "<tr style=\"background-color:orange;color:#FFFFFF;font-weight: bold;\">\n";
 		                                echo "  <td> </td>\n";
        		                        //echo "  <td>".utf8_decode($prvek["jmeno"])."</td>\n";

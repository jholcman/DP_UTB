...
//////////////////////
$nazev_akce = htmlspecialchars($_POST['nazev'], ENT_QUOTES);
$etapy = [];
$subpole = [];
$ucastnici = [];
$last_id_akce = 0;
$last_id_trate = 0;
$url_akce = htmlspecialchars($_POST['url'], ENT_QUOTES);
...
$sql = "SELECT * FROM akce WHERE nazev='".utf8_encode($nazev_akce)."'";
...
if (!$html) {
  ...
  $sql_akce =  "SELECT max(poradi) AS maximum FROM akce";
  $result_akce = mysqli_query($conn, $sql_akce);
  $row_akce = mysqli_fetch_assoc($result_akce);
  $maximalniporadi = $row_akce['maximum'] + 1;
  
  $sql_akce =  "INSERT INTO akce (nazev,url_akce,url_mapa,poradi) VALUES ('".utf8_encode($nazev_akce)."','".$url_akce."','".$url_mapa."',".$maximalniporadi.")";
  mysqli_query($conn, $sql_akce);
  $last_id_akce = mysqli_insert_id($conn);
